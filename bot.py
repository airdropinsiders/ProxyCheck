...
class ProxyChecker:
    ...
    """
        print(banner)
        print(f"{Fore.YELLOW}STATIC PROXY QUALITY CHECKER")
    ...
    def main(self):
        ...
        while True:
            try:
                input_file = input(f"{Fore.GREEN}Proxy list filename: {Style.RESET_ALL}").strip()
                ...
                continue_choice = input(f"{Fore.YELLOW}Do you want to process another proxies? (y/n): {Style.RESET_ALL}").strip().lower()
